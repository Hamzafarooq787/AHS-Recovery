  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-center gap-3">
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="flex items-center justify-center w-12 h-12 md:w-[60px] md:h-[60px] rounded-full bg-primary hover:brightness-110 text-background-dark shadow-lg hover:shadow-xl transition-all"
        aria-label={open ? "Close call and WhatsApp options" : "Call or WhatsApp AHS Recovery"}
        aria-expanded={open}
      >
        {open ? <X className="w-5 h-5 md:w-6 md:h-6" /> : <Phone className="w-5 h-5 md:w-6 md:h-6" />}
      </button>
      <a
